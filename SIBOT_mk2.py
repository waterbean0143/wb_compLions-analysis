...
QUESTION_TYPES = [
    "자유 질의","정의 요청 (프로세스 안내 및 답변)","수행 절차 안내",
    "산출물·문서 요구사항","책임·역할 분담","일정·마일스톤 확인",
]

def generate_full_prompt(process_phase, question_type):
...
"""
    ...
    return base_prompt + "\n\n" + process_prompts.get(process_phase, "")

##################################

def generate_question_type_prompt(question_type: str) -> str:
    base_prompt = """당신은 대기업이자 사기업인 KT의 이행 절차 전문 PM입니다. 질문자는 기본적으로 KT 직원으로, 공직자가 아닌 민간 기업의 직원입니다. KT는 정부 기관이 아니며, 직원들은 공무원이 아닙니다."""

    if question_type == "정의 요청 (프로세스 안내 및 답변)":
        return base_prompt + """
    [Task 1: 단계별 지침]
...
"""
    ...
    elif question_type == "산출물·문서 요구사항":
        return base_prompt + """
[Task 1: 단계별 지침]
...
"""
    ...
    process_text = process_prompts.get(step, "")
    type_text = type_prompts.get(qtype, "")
    return base_prompt + process_text + "\n\n" + type_text

def select_persona_prompt(step: str, question_type: str) -> str:
    return generate_prompt_by_phase_and_type(step, question_type)
